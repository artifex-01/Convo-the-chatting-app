      ...
      <div className="pt-8 pb-4 px-6 bg-[#F2F4F7]">
        {isSearchActive ? (
           <div className="flex items-center justify-between mb-6 animate-[slideInSearch_0.3s_cubic-bezier(0.16,1,0.3,1)] origin-top">
            <div className="flex-1 flex items-center bg-white rounded-full px-4 py-3 shadow-lg border border-gray-100 focus-within:ring-2 focus-within:ring-black/5 transition-all">
               ...
               />
               {searchQuery && (
                 <button onClick={() => setSearchQuery('')} className="bg-gray-200 rounded-full p-1 hover:bg-gray-300 transition-colors animate-[fadeIn_0.2s]">
            ...
            <button 
                ...
                className="ml-3 text-sm font-bold text-gray-500 hover:text-gray-800 transition-colors whitespace-nowrap px-2 py-1 hover:bg-gray-200 rounded-lg"
            >
                Cancel
            </button>
          </div>
          ...
          <div className="flex items-start justify-between mb-6 animate-[fadeIn_0.3s_ease-out]">
            ...
            <div className="flex items-center gap-4 text-gray-500">
              <button 
                ...
                className="p-2 hover:bg-white hover:shadow-sm rounded-full transition-all hover:scale-110 active:scale-95"
              >
                  <SearchIcon className="w-6 h-6" />
              </button>
              <button className="p-2 hover:bg-white hover:shadow-sm rounded-full transition-all hover:scale-110 active:scale-95">
          ...
          {[
            ...
            <button
              ...
              className={`pb-3 text-[15px] relative transition-all duration-200 ${
                activeFilter === tab.id 
                  ? 'font-bold text-[#0F172A] border-b-2 border-black -mb-[1px]' 
                  : 'font-medium text-[#94A3B8] hover:text-gray-600 border-transparent'
              }`}
            >
              {tab.label}
            </button>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-32 no-scrollbar space-y-3">
        {filteredChats.length > 0 ? (
          filteredChats.map((chat, index) => {
            ...
            return (
              <div
                ...
                className="bg-white rounded-[20px] p-4 flex items-center gap-4 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] cursor-pointer active:scale-[0.98] transition-transform animate-[fadeIn_0.3s_ease-out_fill-mode-backwards]"
          ...
          <div className="flex flex-col items-center justify-center h-64 text-gray-400 text-sm animate-[fadeIn_0.3s_ease-out]">
             ...
             <p className="font-bold text-gray-500 text-lg">No results found</p>
             <p className="text-xs mt-2 text-gray-400">Try searching for something else</p>
          </div>
        )}
      </div>
